...
> 关于 `deploy` 命令的常见问题和解决方法，可以参考[ FC 组件自动问答系统](http://qa.devsapp.cn/ ) 。

> 关于 [如何部署多个函数](./../tips.md#如何部署多个函数), [超过50M的代码包如何部署](./../tips.md#超过50M的代码包如何部署) 等问题，请参考 [Tips 文档](./../tips.md) 。
...
- **在部署时可能会涉及到交互式操作**：
  ...
  - 当检测到线上的资源与本地所记录的上次部署的资源不一致时，会提醒是否使用线上资源还是使用线下资源；如果不想出现该交互式操作，可以在执行命令时增加`--use-local`/`--use-remote`参数，此时会优先使用本地配置/线上配置，例如：
    ...
    | 线上服务 | 线上函数 | 本地记录的服务 | 本地记录的函数 | 直接部署 | `use-local`              | `use-remote`｜                       |
    | -------- | -------- | -------------- | -------------- | -------- | ------------------------ | ------------------------------------ |
