...
- **全生命周期管理**：组件拥有项目的创建、开发、调试、部署、运维全生命周期管理能力；支持通过 Yaml 对资源进行描述与纯命令行形式的多种使用方法；可以在从项目的部署运维，应用和资源的管理等多种场景进行友好的支持；
...
- **可观测性**：组件拥有相对完善的可观测性能力，在客户端可以通过`metrics`以及`log`等命令，进行业务的数据指标、执行日志等多重维度的观测；在`metrics`的能力中，不仅可以查看到函数的基础运行指标，例如执行次数，成功/失败次数，还可以看到冷启动相关的指标内容；
- **多模调试**：创新新的提出了多模调试方案，可以同时满足开发态、运维态的不同调试需求；包括本地运行、在线运行、本地调试、云端调试、端云联调等功能，其中端云联调能力，创新性的将线上环境映射到本地，在本地即可进行项目的开发和调试；
